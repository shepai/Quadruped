...
class newGait(CTRNNQuadruped):
    def mutate(self,rate=0.2):
        #evolveable parameters=3 dt, height, phase
        noise=np.random.normal(0,0.1,(2+self.num_motors))
...
def RUN(sho=0,trial=0,generations=300,fit=fitness_,fric=0.5):
    ...
    #play the trials on reapeat
        if gen%10==0:
            with open(datapath+'/models/genotypes_dt'+"_"+str(trial)+str(fric)+'.pkl', 'wb') as f:
                pickle.dump(population, f)
            np.save(datapath+'/models/fitnesses_dt'+"_"+str(trial)+str(fric),fitnesses)
            np.save(datapath+'/models/history_dt'+"_"+str(trial)+str(fric),history)
...
if __name__=="__main__":
    ...
    for i in range(0,40):
        for j in np.arange(0,1,0.05):
            ...
            RUN(sho=0,trial="evolving_hyperparamters"+str(i),fit=F1,fric=j)
            c+=1
